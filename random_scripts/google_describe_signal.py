...
answers for so many of these questions that I'm better off with them
ignoring me anyway. Google BARD is garbage at actually
troubleshooting board problems due to lack of training data, but it's
...
get token by hitting F12 while on bard.google.comdo you work th
go to cookies, find `__Secure-1PSID` and copy that into here'''

token = ''
bard = Bard(token=token)  # Fetch a response to a sample question, make sure it works
...
columns = ['NAME', 'DESCRIPTION', 'NEWDESCRIPTION']
signal_df = pd.DataFrame(columns=columns)

signal_df['NAME'] = pd.Series(dtype='object')
signal_df['DESCRIPTION'] = pd.Series(dtype='object')
signal_df['NEWDESCRIPTION'] = pd.Series(dtype='object')

csvfile = "/home/louis/board_repair_training/jargon_lists/signal_bardfull.csv"
...
for index, row in signal_df.iterrows():
    if pd.isna(row['DESCRIPTION']) and pd.isna(row['NEWDESCRIPTION']):
        ...
        if 'Response Error' in response['content']:
            ...
            sleep_time = random.uniform(45, 80)
            ...
            total_count = len(signal_df[(pd.isna(signal_df['DESCRIPTION'])) & (pd.isna(signal_df['NEWDESCRIPTION']))])
